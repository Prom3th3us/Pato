...
case class ObligacionTributariaTransactionBilletera(actorRef: ActorRef, monitoring: Monitoring)(
  ...
  def topic = "DGR-COP-OBLIGACIONES-TRI-BILLETERA"
  def topicRetry = "DGR-COP-OBLIGACIONES-TRI-BILLETERA_retry"
  def topicError = "DGR-COP-OBLIGACIONES-TRI-BILLETERA_error"

  def processInput(input: String): Either[Throwable, ObligacionesTri] =
  {
    //connOracleNifi(input, "obligacion", "DGR-COP-OBLIGACIONES-TRI-BILLETERA")
    maybeDecode[ObligacionesTri](input)
  }

  def processMessage(obligacion: ObligacionesTri): Future[Response.SuccessProcessing] = {
    //log.debug("KW oracle BILLETERA ")
